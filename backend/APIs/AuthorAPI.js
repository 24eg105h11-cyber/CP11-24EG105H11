...
authorApp.put("/articles", verifyToken("AUTHOR"), async (req, res) => {
  try {
    ...
    if (!modifiedArticle) {
      return res.status(403).json({ message: "Not authorized to edit article or article not found" });
    }
    //send res
    res.status(200).json({ message: "Article modified", payload: modifiedArticle });
  } catch (err) {
    res.status(500).json({ message: "Error in editing article", error: err.message });
  }
});
